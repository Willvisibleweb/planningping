...
export default async function CoveragePage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-semibold text-[#202124] mb-1">Coverage</h2>
        <p className="text-sm text-[#6B6C70] max-w-2xl">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 max-w-lg">
        <div className="rounded-[10px] border border-[#D6E4FB] bg-white p-3.5">
          <p className="text-[11px] font-semibold uppercase tracking-wider text-[#A0A1A6]">Authorities</p>
          <p className="mt-1.5 text-2xl font-bold tabular-nums text-[#202124]">{rows.length}</p>
        </div>
        <div className="rounded-[10px] border border-[#D6E4FB] bg-white p-3.5">
          <p className="text-[11px] font-semibold uppercase tracking-wider text-[#A0A1A6]">Applications</p>
          <p className="mt-1.5 text-2xl font-bold tabular-nums text-[#202124]">{totalApplications.toLocaleString()}</p>
        </div>
      </div>

      <div className="rounded-lg border border-[#D6E4FB] bg-white overflow-hidden">
        <div className="grid grid-cols-[1fr_auto_auto] gap-4 border-b border-[#E9F0FD] bg-[#F7F7F8] px-5 py-2.5 text-[11px] font-semibold uppercase tracking-wider text-[#A0A1A6]">
        ...
        <div className="divide-y divide-[#E9F0FD]">
          {rows.map((r) => (
            <div key={r.slug} className="grid grid-cols-[1fr_auto_auto] items-center gap-4 px-5 py-2.5">
              <span className="text-sm text-[#202124]">{r.name}</span>
              <span className="text-right text-sm tabular-nums text-[#6B6C70]">{r.applicationCount.toLocaleString()}</span>
              <span className="text-right">
                {r.hasPublicPage ? (
                  <a href={`/planning-applications/${r.slug}`} className="text-xs font-medium text-[#2563EB] hover:underline">
                    View &rarr;
                  </a>
                ) : (
                  <span className="text-xs text-[#A0A1A6]">&mdash;</span>
                )}
              </span>
            </div>
          ))}
          {rows.length === 0 && (
            <p className="px-5 py-6 text-sm text-[#A0A1A6]">No coverage data yet.</p>
          )}
        </div>
      </div>
    </div>
