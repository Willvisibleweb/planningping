...
export default function ApplicationSearchList({
  ...
  if (items.length === 0) {
    return <p className="text-xs text-ink-muted">No planning applications found in this territory yet.</p>
  }
    ...
    <div>
      <div className="relative mb-3">
        ...
        <input
          ...
          className="w-full rounded-md border border-border py-1.5 pl-8 pr-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
        />
      </div>

      {filtered.length === 0 ? (
        <p className="text-xs text-ink-muted">No applications match &ldquo;{query}&rdquo;.</p>
      ) : (
